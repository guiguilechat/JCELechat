...
public class TypeRegionDateHistoryService {

	private final TypeRegionDateHistoryRepository repo;

	@Autowired
	private DataSource datasource;

	public void insertAll(List<TypeRegionDateHistory> entities) {
		try {
	...
	protected boolean insertPGCopy(List<TypeRegionDateHistory> entities, CopyManager cm) {
		long start = System.currentTimeMillis();
		StringReader reader =
				new StringReader(
						entities.stream()
								.map(TypeRegionDateHistory::csv)
								.collect(Collectors.joining("\n")));
		long postReader = System.currentTimeMillis();
		try {
			cm.copyIn("COPY esi_trade_typeregiondatehistory (" + TypeRegionDateHistory.CSV_HEADER
