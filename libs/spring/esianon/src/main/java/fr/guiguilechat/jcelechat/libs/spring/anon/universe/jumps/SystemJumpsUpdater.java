...
@Setter
@Service
public class SystemJumpsUpdater implements EntityUpdater {

	@Lazy
	private final SystemJumpsRepository repo;
	...
	public boolean updatePulse() {
		...
		Requested<R_get_universe_system_jumps[]> ret =
				ESIRawPublic.INSTANCE
						.get_universe_system_jumps(null);
		if (ret.isOk()) {
			...
			if (!lastModified.equals(previousLastModified)) {
				...
				var translated =
						Stream.of(ret.getOK())
								.map(r -> new SystemJumps(r.system_id, periodEnd, periodStart, periodMid, r.ship_jumps))
								.toList();
