package fr.guiguilechat.jcelechat.libs.spring.sde.space.asteroidbelt;

import java.math.BigDecimal;
import java.util.function.Function;

import org.hibernate.annotations.Cache;
import org.hibernate.annotations.CacheConcurrencyStrategy;

import fr.guiguilechat.jcelechat.libs.sde.cache.parsers.EmapAsteroidBelts;
import fr.guiguilechat.jcelechat.libs.spring.sde.items.type.Type;
import fr.guiguilechat.jcelechat.libs.spring.sde.space.generic.SdeInPlanetOrbit;
import fr.guiguilechat.jcelechat.libs.spring.sde.space.planet.Planet;
import fr.guiguilechat.jcelechat.libs.spring.sde.space.solarsystem.SolarSystem;
import jakarta.persistence.Entity;
import jakarta.persistence.Index;
import jakarta.persistence.ManyToOne;
import jakarta.persistence.Table;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Entity(name = "SdeSpaceAsteroidBelt")
@Table(name = "sde_space_asteroidbelt", indexes = {
		@Index(columnList = "solar_system_id"),
		@Index(columnList = "type_id")
})
@Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
@Getter
@Setter
@NoArgsConstructor
public class AsteroidBelt extends SdeInPlanetOrbit {

	@ManyToOne
	private Planet planet;
	private BigDecimal radius;
	private String uniqueName;

	public void update(EmapAsteroidBelts source,
			Function<Integer, Type> types,
			Function<Integer, SolarSystem> solarSystems,
			Function<Integer, Planet> planets) {
		super.update(source, types, solarSystems);
		setPlanet(planets.apply(source.orbitID));
		setRadius(source.radius);
		setUniqueName(source.enUniqueName());
	}

}
