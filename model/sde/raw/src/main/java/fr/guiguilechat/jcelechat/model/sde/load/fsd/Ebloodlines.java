...
public class Ebloodlines {
	...
	public static final String SDE_FILE = "fsd/bloodlines.yaml";

	public static final JacksonYamlLoader<LinkedHashMap<Integer, Ebloodlines>> LOADER_JACKSON = new JacksonYamlLoader<>(
			SDE_FILE);

	public static final SnakeYamlLHMLoader<Integer, Ebloodlines> LOADER_SNAKEYAML = new SnakeYamlLHMLoader<>(SDE_FILE) {
			...
			if (node.getNodeId() == NodeId.mapping) {
				...
				if (mn.getValue().size() > 0) {
					if (mn.getValue().stream().map(nt -> ((ScalarNode) nt.getKeyNode()).getValue())
							.filter("descriptionID"::equals).findAny().isPresent()) {
