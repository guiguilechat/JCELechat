    ...
    extends Item
{
    ...
    public static Map<String, ? extends Lights> loadCategory() {
        return Stream.of(PointLights.load(), BoxLights.load(), SpotLights.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
