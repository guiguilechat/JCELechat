    ...
    extends Item
{
    ...
    public static Map<String, ? extends AncientRelics> loadCategory() {
        return Stream.of(SleeperElectronicsRelics.load(), SleeperOffensiveRelics.load(), SleeperHullRelics.load(), SleeperPropulsionRelics.load(), SleeperEngineeringRelics.load(), SleeperDefensiveRelics.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
