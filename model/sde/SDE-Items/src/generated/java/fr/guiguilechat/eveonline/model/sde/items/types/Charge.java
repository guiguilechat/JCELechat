    ...
    extends Item
{
    ...
    public static Map<String, ? extends Charge> loadCategory() {
        return Stream.of(AdvancedArtilleryAmmo.load(), AdvancedLightMissile.load(), AdvancedPulseLaserCrystal.load(), XLCruiseMissile.load(), FrequencyCrystal.load(), ScannerProbe.load(), CompactXLTorpedo.load(), StructureResistanceSwitcherScript.load(), CapacitorBoosterCharge.load(), FlexArmorHardenerScript.load(), StructureAntiCapitalMissile.load(), MiningCrystal.load(), HeavyDefenderMissile.load(), InterdictionProbe.load(), AdvancedXLCruiseMissile.load(), SurveyProbe.load(), MercoxitMiningCrystal.load(), AdvancedRailgunCharge.load(), DefenderMissiles.load(), Modifications.load(), AdvancedTorpedo.load(), BombECM.load(), XLTorpedo.load(), AutoTargetingLightMissile.load(), AdvancedRocket.load(), ArmorCommandBurstCharges.load(), TrackingScript.load(), SensorBoosterScript.load(), TrackingDisruptionScript.load(), AutoTargetingHeavyMissile.load(), HeavyAssaultMissile.load(), WarpDisruptionScript.load(), PlanetSatellites.load(), StructureAntiSubcapitalMissile.load(), AdvancedAutocannonAmmo.load(), LightMissile.load(), GuidanceDisruptionScript.load(), NaniteRepairPaste.load(), AdvancedXLTorpedo.load(), MiningForemanBurstCharges.load(), OrbitalAssaultUnit.load(), Torpedo.load(), Mine.load(), AdvancedBlasterCharge.load(), ShieldCommandBurstCharges.load(), ObsoleteProbes.load(), SensorDampenerScript.load(), MissileGuidanceScript.load(), Bomb.load(), FestivalCharges.load(), AdvancedBeamLaserCrystal.load(), StructureECMScript.load(), BombEnergy.load(), AutoTargetingCruiseMissile.load(), ShieldBoosterScript.load(), InformationCommandBurstCharges.load(), ProjectileAmmo.load(), FlexShieldHardenerScript.load(), Rocket.load(), AdvancedCruiseMissile.load(), AdvancedHeavyAssaultMissile.load(), AdvancedHeavyMissile.load(), HeavyMissile.load(), StructureGuidedBomb.load(), SkirmishCommandBurstCharges.load(), HybridCharge.load(), CruiseMissile.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
