...
import fr.guiguilechat.eveonline.model.sde.items.types.charge.StructureWarpDisruptorScript;
    ...
    extends Item
{
    ...
    public static Map<String, ? extends Charge> loadCategory() {
        return Stream.of(AdvancedArtilleryAmmo.load(), AdvancedAutocannonAmmo.load(), AdvancedBeamLaserCrystal.load(), AdvancedBlasterCharge.load(), AdvancedCruiseMissile.load(), AdvancedHeavyAssaultMissile.load(), AdvancedHeavyMissile.load(), AdvancedLightMissile.load(), AdvancedPulseLaserCrystal.load(), AdvancedRailgunCharge.load(), AdvancedRocket.load(), AdvancedTorpedo.load(), AdvancedXLCruiseMissile.load(), AdvancedXLTorpedo.load(), ArmorCommandBurstCharges.load(), AutoTargetingCruiseMissile.load(), AutoTargetingHeavyMissile.load(), AutoTargetingLightMissile.load(), Bomb.load(), BombECM.load(), BombEnergy.load(), CapacitorBoosterCharge.load(), CompactXLTorpedo.load(), CruiseMissile.load(), DefenderMissiles.load(), FestivalCharges.load(), FlexArmorHardenerScript.load(), FlexShieldHardenerScript.load(), FrequencyCrystal.load(), GuidanceDisruptionScript.load(), HeavyAssaultMissile.load(), HeavyDefenderMissile.load(), HeavyMissile.load(), HybridCharge.load(), InformationCommandBurstCharges.load(), InterdictionProbe.load(), LightMissile.load(), MercoxitMiningCrystal.load(), Mine.load(), MiningCrystal.load(), MiningForemanBurstCharges.load(), MissileGuidanceScript.load(), Modifications.load(), NaniteRepairPaste.load(), ObsoleteProbes.load(), OrbitalAssaultUnit.load(), PlanetSatellites.load(), ProjectileAmmo.load(), Rocket.load(), ScannerProbe.load(), SensorBoosterScript.load(), SensorDampenerScript.load(), ShieldBoosterScript.load(), ShieldCommandBurstCharges.load(), SkirmishCommandBurstCharges.load(), StructureAntiCapitalMissile.load(), StructureAntiSubcapitalMissile.load(), StructureECMScript.load(), StructureGuidedBomb.load(), StructureResistanceSwitcherScript.load(), StructureWarpDisruptorScript.load(), SurveyProbe.load(), Torpedo.load(), TrackingDisruptionScript.load(), TrackingScript.load(), WarpDisruptionScript.load(), XLCruiseMissile.load(), XLTorpedo.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
