    ...
    extends Item
{
    ...
    public static Map<String, ? extends Abstrct> loadCategory() {
        return Stream.of(Audio.load(), Decorations.load(), PerceptionPoints.load(), Miscellaneous.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
