    ...
    extends Item
{
    ...
    public static Map<String, ? extends Ship> loadCategory() {
        return Stream.of(Titan.load(), Carrier.load(), BlackOps.load(), Frigate.load(), StealthBomber.load(), HeavyAssaultCruiser.load(), TacticalDestroyer.load(), ElectronicAttackShip.load(), CommandShip.load(), Exhumer.load(), ForceReconShip.load(), CommandDestroyer.load(), Corvette.load(), Logistics.load(), Battleship.load(), Industrial.load(), BlockadeRunner.load(), Supercarrier.load(), CapitalIndustrialShip.load(), PrototypeExplorationShip.load(), IndustrialCommandShip.load(), ForceAuxiliary.load(), Destroyer.load(), JumpFreighter.load(), Shuttle.load(), CombatReconShip.load(), ExpeditionFrigate.load(), CovertOps.load(), Interdictor.load(), Freighter.load(), Marauder.load(), StrategicCruiser.load(), AttackBattlecruiser.load(), LogisticsFrigate.load(), MiningBarge.load(), Capsule.load(), AssaultFrigate.load(), DeepSpaceTransport.load(), CombatBattlecruiser.load(), Cruiser.load(), Interceptor.load(), Dreadnought.load(), HeavyInterdictionCruiser.load()).flatMap((m -> m.entrySet().stream())).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
