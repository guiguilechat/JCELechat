...
public class Corporations {
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_bookmarks_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_bookmarks",
							(page, headerHandler) -> cache.swagger.get_corporations_bookmarks(corporation_id, page, headerHandler),
							arr -> {
	...
	public ObsListHolder<R_get_corporations_corporation_id_bookmarks_folders> bookmarks_folders(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_bookmarks_folders> ret = get_corporations_corporation_id_bookmarks_folders_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_bookmarks_folders_holder) {
				...
				if (ret == null) {
					ObservableList<R_get_corporations_corporation_id_bookmarks_folders> holder = FXCollections
							.observableArrayList();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_bookmarks_folders", (page,
							headerHandler) -> cache.swagger.get_corporations_bookmarks_folders(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<M_get_contacts_labels_2> contacts_labels(int corporation_id) {
		ObsListHolder<M_get_contacts_labels_2> ret = get_corporations_corporation_id_contacts_labels_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_contacts_labels_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_contacts_labels",
							(page, headerHandler) -> cache.swagger.get_corporations_contacts_labels(corporation_id, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	 ...
	 * issuer, acceptor or assignee. Only returns contracts no older than 30 days,
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_contracts_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_contracts",
							(page, headerHandler) -> cache.swagger.get_corporations_contracts(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsMapHolder<Integer, M_get_contracts_contract_bids_4> contracts_bids(int contract_id, int corporation_id) {
		...
		ObsMapHolder<Integer, M_get_contracts_contract_bids_4> ret = get_corporations_corporation_id_contracts_contract_id_bids_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_contracts_contract_id_bids_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_contracts_contract_id_bids",
							(page, headerHandler) -> cache.swagger.get_corporations_contracts_bids(contract_id, corporation_id, page,
									headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
	...
	public ObsMapHolder<Long, M_get_contracts_contract_items_6> contracts_items(int contract_id, int corporation_id) {
		K_8_int_int param = new K_8_int_int(corporation_id, contract_id);
		ObsMapHolder<Long, M_get_contracts_contract_items_6> ret = get_corporations_corporation_id_contracts_contract_id_items_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_contracts_contract_id_items_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_contracts_contract_id_items",
							(page, headerHandler) -> cache.swagger.get_corporations_contracts_items(contract_id, corporation_id,
									headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
	...
	public ObsMapHolder<Long, R_get_corporations_corporation_id_customs_offices> customs_offices(int corporation_id) {
		ObsMapHolder<Long, R_get_corporations_corporation_id_customs_offices> ret = get_corporations_corporation_id_customs_offices_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_customs_offices_holder) {
				...
				if (ret == null) {
					ObservableMap<Long, R_get_corporations_corporation_id_customs_offices> holder = FXCollections
							.observableHashMap();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_customs_offices", (page,
							headerHandler) -> cache.swagger.get_corporations_customs_offices(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Return corporation hangar and wallet division names, only show if a
	 * division is not using the default name
	 *
	...
	public ObsObjHolder<R_get_corporations_corporation_id_divisions> divisions(int corporation_id) {
		ObsObjHolder<R_get_corporations_corporation_id_divisions> ret = get_corporations_corporation_id_divisions_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_divisions_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheObject("get_corporations_corporation_id_divisions",
							headerHandler -> cache.swagger.get_corporations_divisions(corporation_id, headerHandler), item -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.set(item);
	...
	public ObsListHolder<R_get_corporations_corporation_id_facilities> facilities(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_facilities> ret = get_corporations_corporation_id_facilities_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_facilities_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_facilities",
							(page, headerHandler) -> cache.swagger.get_corporations_facilities(corporation_id, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsObjHolder<R_get_corporations_corporation_id_fw_stats> fw_stats(int corporation_id) {
		ObsObjHolder<R_get_corporations_corporation_id_fw_stats> ret = get_corporations_corporation_id_fw_stats_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_fw_stats_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheObject("get_corporations_corporation_id_fw_stats",
							headerHandler -> cache.swagger.get_corporations_fw_stats(corporation_id, headerHandler), item -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.set(item);
	 ...
	 *          includes jobs from the past 90 days
	 */
	public ObsMapHolder<Integer, R_get_corporations_corporation_id_industry_jobs> industry_jobs(int corporation_id,
			Boolean include_completed) {
		K_9_int_Boolean param = new K_9_int_Boolean(corporation_id, include_completed);
		ObsMapHolder<Integer, R_get_corporations_corporation_id_industry_jobs> ret = get_corporations_corporation_id_industry_jobs_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_industry_jobs_holder) {
				...
				if (ret == null) {
					ObservableMap<Integer, R_get_corporations_corporation_id_industry_jobs> holder = FXCollections
							.observableHashMap();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_industry_jobs",
							(page, headerHandler) -> cache.swagger.get_corporations_industry_jobs(corporation_id, include_completed,
									page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_killmails_recent_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_killmails_recent", (page,
							headerHandler) -> cache.swagger.get_corporations_killmails_recent(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<R_get_corporations_corporation_id_medals> medals(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_medals> ret = get_corporations_corporation_id_medals_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_medals_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_medals",
							(page, headerHandler) -> cache.swagger.get_corporations_medals(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<R_get_corporations_corporation_id_medals_issued> medals_issued(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_medals_issued> ret = get_corporations_corporation_id_medals_issued_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_medals_issued_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_medals_issued", (page,
							headerHandler) -> cache.swagger.get_corporations_medals_issued(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_members_limit_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheObject("get_corporations_corporation_id_members_limit",
							headerHandler -> cache.swagger.get_corporations_members_limit(corporation_id, headerHandler), item -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.set(item);
	...
	public ObsListHolder<R_get_corporations_corporation_id_members_titles> members_titles(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_members_titles> ret = get_corporations_corporation_id_members_titles_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_members_titles_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_members_titles",
							(page, headerHandler) -> cache.swagger.get_corporations_members_titles(corporation_id, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Returns additional information about a corporation's members which helps
	...
	public ObsListHolder<R_get_corporations_corporation_id_membertracking> membertracking(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_membertracking> ret = get_corporations_corporation_id_membertracking_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_membertracking_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_membertracking",
							(page, headerHandler) -> cache.swagger.get_corporations_membertracking(corporation_id, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Return the roles of all members if the character has the personnel manager
	 * role or any grantable role.
	 *
	...
	public ObsListHolder<R_get_corporations_corporation_id_roles> roles(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_roles> ret = get_corporations_corporation_id_roles_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_roles_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_roles",
							(page, headerHandler) -> cache.swagger.get_corporations_roles(corporation_id, headerHandler), arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.setAll(arr);
	...
	public ObsListHolder<R_get_corporations_corporation_id_roles_history> roles_history(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_roles_history> ret = get_corporations_corporation_id_roles_history_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_roles_history_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_roles_history", (page,
							headerHandler) -> cache.swagger.get_corporations_roles_history(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<R_get_corporations_corporation_id_shareholders> shareholders(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_shareholders> ret = get_corporations_corporation_id_shareholders_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_shareholders_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_shareholders",
							(page, headerHandler) -> cache.swagger.get_corporations_shareholders(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_standings_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_standings",
							(page, headerHandler) -> cache.swagger.get_corporations_standings(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsMapHolder<Long, R_get_corporations_corporation_id_starbases> starbases(int corporation_id) {
		ObsMapHolder<Long, R_get_corporations_corporation_id_starbases> ret = get_corporations_corporation_id_starbases_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_starbases_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_starbases",
							(page, headerHandler) -> cache.swagger.get_corporations_starbases(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	 ...
	 */
	public ObsObjHolder<R_get_corporations_corporation_id_starbases_starbase_id> starbases(int corporation_id,
			long starbase_id, int system_id) {
		K_10_int_long_int param = new K_10_int_long_int(corporation_id, starbase_id, system_id);
		ObsObjHolder<R_get_corporations_corporation_id_starbases_starbase_id> ret = get_corporations_corporation_id_starbases_starbase_id_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_starbases_starbase_id_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheObject("get_corporations_corporation_id_starbases_starbase_id",
							headerHandler -> cache.swagger.get_corporations_starbases(corporation_id, starbase_id, system_id,
									headerHandler),
							item -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
	...
	public ObsListHolder<R_get_corporations_corporation_id_titles> titles(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_titles> ret = get_corporations_corporation_id_titles_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_titles_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_titles",
							(page, headerHandler) -> cache.swagger.get_corporations_titles(corporation_id, headerHandler), arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.setAll(arr);
	...
	public ObsListHolder<R_get_corporations_corporation_id_wallets> wallets(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_wallets> ret = get_corporations_corporation_id_wallets_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_wallets_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_wallets",
							(page, headerHandler) -> cache.swagger.get_corporations_wallets(corporation_id, headerHandler), arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
									holder.setAll(arr);
	 ...
	 */
	public ObsMapHolder<Long, R_get_corporations_corporation_id_wallets_division_transactions> wallets_transactions(
			int corporation_id, int division, Long from_id) {
		K_11_int_int_Long param = new K_11_int_int_Long(division, corporation_id, from_id);
		ObsMapHolder<Long, R_get_corporations_corporation_id_wallets_division_transactions> ret = get_corporations_corporation_id_wallets_division_transactions_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_wallets_division_transactions_holder) {
				...
				if (ret == null) {
					ObservableMap<Long, R_get_corporations_corporation_id_wallets_division_transactions> holder = FXCollections
							.observableHashMap();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_wallets_division_transactions",
							(page, headerHandler) -> cache.swagger.get_corporations_wallets_transactions(corporation_id, division,
									from_id, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsMapHolder<Long, R_get_corporations_corporation_id_blueprints> blueprints(int corporation_id) {
		ObsMapHolder<Long, R_get_corporations_corporation_id_blueprints> ret = get_corporations_corporation_id_blueprints_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_blueprints_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_blueprints",
							(page, headerHandler) -> cache.swagger.get_corporations_blueprints(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<R_get_corporations_corporation_id_contacts> contacts(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_contacts> ret = get_corporations_corporation_id_contacts_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_contacts_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_contacts",
							(page, headerHandler) -> cache.swagger.get_corporations_contacts(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Returns logs recorded in the past seven days from all audit log secure
	 * containers (ALSC) owned by a given corporation
	 *
	...
	public ObsListHolder<R_get_corporations_corporation_id_containers_logs> containers_logs(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_containers_logs> ret = get_corporations_corporation_id_containers_logs_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_containers_logs_holder) {
				...
				if (ret == null) {
					ObservableList<R_get_corporations_corporation_id_containers_logs> holder = FXCollections
							.observableArrayList();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_containers_logs", (page,
							headerHandler) -> cache.swagger.get_corporations_containers_logs(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsMapHolder<Long, R_get_corporations_corporation_id_orders_history> orders_history(int corporation_id) {
		ObsMapHolder<Long, R_get_corporations_corporation_id_orders_history> ret = get_corporations_corporation_id_orders_history_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_orders_history_holder) {
				...
				if (ret == null) {
					ObservableMap<Long, R_get_corporations_corporation_id_orders_history> holder = FXCollections
							.observableHashMap();
					ret = cache.toHolder(holder);
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_orders_history", (page,
							headerHandler) -> cache.swagger.get_corporations_orders_history(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	 ...
	 * changes to structures detailed in this blog:
	...
	public ObsListHolder<R_get_corporations_corporation_id_structures> structures(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_structures> ret = get_corporations_corporation_id_structures_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_structures_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_structures",
							(page, headerHandler) -> cache.swagger.get_corporations_structures(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	public ObsListHolder<R_get_corporations_corporation_id_assets> assets(int corporation_id) {
		ObsListHolder<R_get_corporations_corporation_id_assets> ret = get_corporations_corporation_id_assets_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_assets_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_assets",
							(page, headerHandler) -> cache.swagger.get_corporations_assets(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Return the current member list of a corporation, the token's character need
		...
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_members_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_members",
							(page, headerHandler) -> IntStream
							.of(cache.swagger.get_corporations_members(corporation_id, headerHandler)).mapToObj(Integer::valueOf)
							.toArray(Integer[]::new),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
	...
	public ObsMapHolder<Long, R_get_corporations_corporation_id_orders> orders(int corporation_id) {
		ObsMapHolder<Long, R_get_corporations_corporation_id_orders> ret = get_corporations_corporation_id_orders_holder
				.get(corporation_id);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_orders_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_orders",
							(page, headerHandler) -> cache.swagger.get_corporations_orders(corporation_id, page, headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
									LockWatchDog.BARKER.hld(holder);
	...
	/**
	 * Retrieve the given corporation's wallet journal for the given division
	 * going 30 days back
	 *
	...
	public ObsMapHolder<Long, M_get_journal_13> wallets_journal(int corporation_id, int division) {
		...
		ObsMapHolder<Long, M_get_journal_13> ret = get_corporations_corporation_id_wallets_division_journal_holder
				.get(param);
		if (ret == null) {
			...
			synchronized (get_corporations_corporation_id_wallets_division_journal_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_corporations_corporation_id_wallets_division_journal",
							(page, headerHandler) -> cache.swagger.get_corporations_wallets_journal(corporation_id, division, page,
									headerHandler),
							arr -> {
								LockWatchDog.BARKER.tak(holder);
								synchronized (holder) {
