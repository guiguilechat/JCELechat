		...
		if (ret == null) {
			...
			synchronized (get_markets_region_id_history_holder)
			{
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_markets_region_id_history_reg=" + region_id + "_type=" + type_id,
							(page, properties) -> cache.swagger.get_markets_history(region_id, type_id, properties), arr -> {
						LockWatchDog.BARKER.tak(holder);
						synchronized (holder)
						{
							LockWatchDog.BARKER.hld(holder);
