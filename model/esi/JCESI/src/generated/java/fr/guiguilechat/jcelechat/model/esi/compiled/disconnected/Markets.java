...
public class Markets {
		...
		if (get_markets_groups_holder == null) {
			synchronized (this) {
				if (get_markets_groups_holder == null) {
					...
					cache.addFetchCacheArray("get_markets_groups", (page, headerHandler) -> IntStream
							.of(cache.swagger.get_markets_groups(headerHandler)).mapToObj(Integer::valueOf).toArray(Integer[]::new),
							arr -> {
								synchronized (holder) {
									holder.setAll(arr);
								}
	...
	public ObsObjHolder<R_get_markets_groups_market_group_id> groups(int market_group_id) {
		ObsObjHolder<R_get_markets_groups_market_group_id> ret = get_markets_groups_market_group_id_holder
				.get(market_group_id);
		if (ret == null) {
			synchronized (get_markets_groups_market_group_id_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheObject("get_markets_groups_market_group_id",
							headerHandler -> cache.swagger.get_markets_groups(market_group_id, headerHandler), item -> {
								synchronized (holder) {
									holder.set(item);
								}
							});
		...
		if (get_markets_prices_holder == null) {
			synchronized (this) {
				if (get_markets_prices_holder == null) {
					...
					cache.addFetchCacheArray("get_markets_prices",
							(page, headerHandler) -> cache.swagger.get_markets_prices(headerHandler), arr -> {
								synchronized (holder) {
									holder.setAll(arr);
								}
							});
		...
		if (ret == null) {
			synchronized (get_markets_region_id_history_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_markets_region_id_history",
							(page, headerHandler) -> cache.swagger.get_markets_history(region_id, type_id, headerHandler), arr -> {
								synchronized (holder) {
									holder.setAll(arr);
								}
							});
	 ...
	 *
	 * @param order_type
	 *          Filter buy/sell orders, return all orders by default. If you query
	 *          without type_id, we always return both buy and sell orders.
	 * @param region_id
	 *          Return orders in this region
	 * @param type_id
	 *          Return orders only for this type
	 */
	public ObsListHolder<R_get_markets_region_id_orders> orders(
			fr.guiguilechat.jcelechat.model.esi.compiled.structures.order_type order_type, int region_id, Integer type_id) {
		...
		if (ret == null) {
			synchronized (get_markets_region_id_orders_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_markets_region_id_orders", (page, headerHandler) -> cache.swagger
							.get_markets_orders(order_type, page, region_id, type_id, headerHandler), arr -> {
								synchronized (holder) {
									holder.setAll(arr);
								}
							});
	...
	/**
	 * Return a list of type IDs that have active orders in the region, for
		...
		if (ret == null) {
			synchronized (get_markets_region_id_types_holder) {
				...
				if (ret == null) {
					...
					cache.addFetchCacheArray("get_markets_region_id_types",
							(page, headerHandler) -> IntStream.of(cache.swagger.get_markets_types(page, region_id, headerHandler))
