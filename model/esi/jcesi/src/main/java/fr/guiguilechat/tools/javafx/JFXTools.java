...
public class JFXTools {
	...
	public static <T, U> ObjectProperty<T> extractObservable(ObservableValue<U> variable,
		...
		synchronized (variable) {
			...
			variable.addListener((ChangeListener<U>) (_, _, newValue) -> {
				ret.unbind();
				if (newValue != null) {
	...
	public static <U> SetHolderImpl<U> hold(ObservableSet<U> o) {
		...
		synchronized (o) {
			o.addListener((SetChangeListener<U>) _ -> ret.set(new HashSet<>(o)));
		}
		return ret;
	}
	...
	public static <K, V> MapHolderImpl<K, V> hold(ObservableMap<K, V> o) {
		...
		synchronized (o) {
			o.addListener((MapChangeListener<K, V>) _ -> ret.set(new HashMap<>(o)));
		}
		return ret;
	}
