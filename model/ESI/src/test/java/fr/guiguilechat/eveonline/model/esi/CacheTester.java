...
import fr.guiguilechat.eveonline.esi.compiled.Swagger.order_type;
import fr.guiguilechat.eveonline.esi.connected.CacheConnected;
...
public class CacheTester {
	...
	public static void main(String[] args) {
		ESIConnected con = new ESIConnected(null, null);


		// noparam-> container
		System.out.println("noparam->container started");
		Property<R_get_status> status = con.cache.status.status();
		CacheConnected.listen(status, (ChangeListener<R_get_status>) (ob, old, now) -> System.out
		...
		CacheConnected.listen(attributes, li -> System.out
				.println("noparam->list " + li.next() + " added " + li.getAddedSubList() + " ; removed " + li.getRemoved()
				+ " hasnext" + li.next()));
		CacheConnected.wait(attributes);
		...
		CacheConnected.listen(orders, (ListChangeListener<R_get_markets_region_id_orders>) li -> System.out.println("params->list "
